...
function updateClientData() {
    ...
    // The UUID box
    var ret = '<br />' + createLinkBox(`${API_LINK}`, 'linregpcr.html', window.uuid, window.isvalid, window.selExperiment, window.selRun);
...
function updateLinRegPCRTable() {
    ...
    if (choiceTable.value == "debug") {
        ...
        var effErrCol = 55;  // "PCR efficiency outlier"
        ...
        for (var row = 0; row < window.linRegPCRTable.length; row++) {
            ...
            if (["std", "pos"].includes(window.linRegPCRTable[row][3])) {  // "sample type"
                ...
                if (!(cqValue > 0.0)) {
                    ...
                    if (window.linRegPCRTable[row][51] == false) {  // "plateau"
                        highlight_ErrPlat = colErr
                    }
                    if (window.linRegPCRTable[row][52] == true) {  // "noisy sample"
